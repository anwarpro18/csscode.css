...
function favouritePosts( json ) {
	...
	for ( var i = 0; i < json.feed.entry.length; i++ ) {
		...
		var item = '<div class="swiper-slide favourite-post"><div class="item-outer"><div class="favourite-thumbnail"><a href="' + postUrl + '"><img src="' + postThumb.replace( '/s72-c/', '/s' + 400 + '/' ) + '"/></a></div><a class="f-category-andalus" href="/search/label/' + postCategory + '?max-results=6">' + postCategory + '</a><div class="favourite-posts-overlay"><h1 class="favourite-posts-title"><a href="' + postUrl + '">' + postTitle + '</a></h1></div></div></div>';
		htmlcode += '</ul>';
		document.write( item );
	}
}
...
$(function() {
	...
	$(window).scroll(function() {
		if ($(window).scrollTop() > aboveHeight) {
			$('#menu-wrap-andalus').addClass('fixed-menu').next();
		} else {
			$('#menu-wrap-andalus').removeClass('fixed-menu').next();
		}
	});
});
      //]]>
